...
if [ "$ENABLE_INIGO_EXTENSION" != "true" ]; then
    log_info "Inigo extension disabled (ENABLE_INIGO_EXTENSION=false)"
    return 0 2>/dev/null || exit 0
fi

log_info "Installing Inigo extension (weeWXWeatherApp support)..."

# Function to validate weewx.conf before proceeding
validate_config() {
    if ! weectl extension list --config=/data/weewx.conf >/dev/null 2>&1; then
        log_error "weewx.conf appears to be corrupted. Attempting restore..."
        source /init/common.sh
        if manage_backups restore; then
            log_success "Config restored successfully from backup"
            return 0
        else
            log_error "Unable to restore config from backup"
            return 1
        fi
    fi
    return 0
...
if ! validate_config; then
    log_error "Cannot proceed with corrupted config file"
...
if [ -n "$INSTALLED_INIGO_VERSION" ] && [ "$INSTALLED_INIGO_VERSION" != "$INIGO_VERSION" ]; then
    log_info "Updating Inigo extension: $INSTALLED_INIGO_VERSION → $INIGO_VERSION"
...
if [ -z "$INSTALLED_INIGO_VERSION" ]; then
    log_info "Installing Inigo extension v$INIGO_VERSION ($UNIT_SYSTEM units)..."
    ...
    log_info "Installing from: $INIGO_URL"
    weectl extension install "$INIGO_URL" --config=/data/weewx.conf --yes
    
    log_success "Inigo extension v$INIGO_VERSION installed successfully"
else
    log_info "Inigo extension v$INIGO_VERSION already installed"
fi

log_success "Inigo installation phase completed"